...
/**
 * Provider → brand slug for the platform SDK's logo set (`@unifiedai/sdk`
 * `getProviderLogo`). The SDK normalizes by lowercasing and stripping dots /
 * whitespace, so these keys match its catalogue. Providers absent here have no
 * brand logo and resolve to the SDK's neutral fallback mark.
 */
const PROVIDER_LOGO_BRAND: Partial<Record<MediaProviderId, string>> = {
  openai: 'openai',
  grok: 'xai',
  nanobanana: 'gemini',
  google: 'google',
...
/**
 * For routed providers (ImageRouter) the upstream brand lives in the model-id
 * prefix (e.g. `openai/gpt-image-2`, `xAI/grok-imagine-video`) rather than the
 * routing provider, so we key the logo on that prefix instead.
 */
const ROUTED_BRAND_BY_PREFIX: Record<string, string> = {
  openai: 'openai',
  google: 'gemini',
  xai: 'xai',
};
...
/**
 * Brand/author name for a media model, keyed to the platform SDK's logo set so
 * the media-model pickers can render the same provider marks as the gateway
 * model picker. Returns undefined when no brand logo exists for the model's
 * provider (the picker then falls back to the SDK's neutral mark).
 */
export function mediaModelLogoAuthor(model: MediaModel): string | undefined {
  if (model.provider === 'imagerouter') {
    const slash = model.id.indexOf('/');
    if (slash > 0) {
      return ROUTED_BRAND_BY_PREFIX[model.id.slice(0, slash).toLowerCase()];
    }
  }
  return PROVIDER_LOGO_BRAND[model.provider];
}

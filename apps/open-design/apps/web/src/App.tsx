...
export function App() {
  ...
  useEffect(() => {
    ...
    // Re-pick when no agent is chosen OR the chosen one isn't available — the
    // latter matters in the UnifiedApp ecosystem, where the daemon surfaces
    // only the platform 'unified' agent, so a config carrying a stale local
    // CLI id (or vice-versa) is corrected to a working agent. Prefer
    // 'unified' whenever the gateway agent is present.
    const currentAvailable = agents.some(
      (a) => a.id === config.agentId && a.available,
    );
    if (config.agentId && currentAvailable) return;
    const preferred =
      agents.find((a) => a.id === 'unified' && a.available) ??
      agents.find((a) => a.available);
    if (!preferred) return;
    if (preferred.id === config.agentId) return;
    setConfig((prev) => {
      if (prev.agentId === preferred.id) return prev;

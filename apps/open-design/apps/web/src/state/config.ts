...
export const DEFAULT_CONFIG: AppConfig = {
  ...
  // Left null so the App's auto-selector picks the first *available* agent
  // once the agents listing lands. Inside the UnifiedApp ecosystem that is
  // the platform-native 'unified' agent (the only one surfaced), so a fresh
  // install works through the gateway with zero provider/key setup; a
  // standalone daemon still falls back to whatever local CLIs are detected.
  agentId: null,
  skillId: null,
  designSystemId: null,

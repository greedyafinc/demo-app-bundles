...
function fieldsForSurface(
  ...
  if (surface === 'image') {
    return [
      ...
      selectField('model', 'Model', IMAGE_MODELS.map((m) => m.id), modelLabels(IMAGE_MODELS), undefined, modelAuthors(IMAGE_MODELS)),
      selectField('ratio', 'Ratio', MEDIA_ASPECTS),
    ];
  }
  if (surface === 'video') {
    const videoModels = VIDEO_MODELS.filter((m) => m.id !== 'hyperframes-html');
    return [
      templateField(surface, promptTemplates),
      selectField('model', 'Model', videoModels.map((m) => m.id), modelLabels(VIDEO_MODELS), undefined, modelAuthors(videoModels)),
      selectField('ratio', 'Ratio', MEDIA_ASPECTS),
  ...
  const fields: InputFieldSpec[] = [
    ...
    selectField('model', 'Model', audioModels.map((m) => m.id), modelLabels(audioModels), undefined, modelAuthors(audioModels)),
...
function selectField(
  ...
  return {
    ...
    ...(placeholder ? { placeholder } : {}),
    optionLabels,
    // Present only for model fields; signals the picker to render a custom
    // logo-bearing listbox (native <option>s can't hold a brand <img>).
...
// Map model id → brand/author name for the SDK logo lookup. Only models whose
// provider has a brand logo are included; the rest fall back to the SDK's
// neutral mark, so the logo column stays present and aligned either way.
function modelAuthors(models: MediaModel[]): Record<string, string> {
  const authors: Record<string, string> = {};
  for (const model of models) {
    const author = mediaModelLogoAuthor(model);
    if (author) authors[model.id] = author;
  }
  return authors;
}

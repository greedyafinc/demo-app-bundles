...
function renderInlinePromptEditor(
  ...
  if (field.type === 'select' && Array.isArray(field.options)) {
    ...
    const optionIcons = optionIconMap(field);
    // Model fields carry per-option brand authors — render a logo-bearing
    // listbox (like UnifiedApp's ModelPicker.vue) instead of a native <select>,
    // whose <option>s can't hold an <img>.
    if (optionIcons) {
      return (
        <ModelOptionList
...
// Per-option brand authors, attached only to model select fields. Returns null
// when absent so the caller falls back to the plain native <select>.
function optionIconMap(field: InputFieldSpec): Record<string, string> | null {
  const icons = (field as { optionIcons?: unknown }).optionIcons;
  return icons && typeof icons === 'object' && !Array.isArray(icons)

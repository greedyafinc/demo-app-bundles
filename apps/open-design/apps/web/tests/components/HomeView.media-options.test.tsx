...
describe('HomeView media composer options', () => {
  ...
  it('renders provider brand logos in the model picker', async () => {
    stubFetch();
    renderHome();
    ...
    const popover = screen.getByTestId('home-hero-prompt-option-model');
    // The model field is now a logo-bearing listbox, not a native <select>.
    expect(within(popover).queryByTestId('home-hero-prompt-option-model-select')).toBeNull();
    expect(within(popover).getAllByRole('option').length).toBeGreaterThan(1);

    const brandedLogo = within(popover)
      .getByRole('option', { name: 'gpt-image-2' })
      .querySelector('img');
    const fallbackLogo = within(popover)
      .getByRole('option', { name: 'flux-1.1-pro' })
      .querySelector('img');

    // Both rows carry a logo image sourced from the SDK (inline data-URIs).
    expect(brandedLogo?.getAttribute('src')).toMatch(/^data:/);
    expect(fallbackLogo?.getAttribute('src')).toMatch(/^data:/);
    // OpenAI resolves to its brand mark; Black Forest Labs has no brand logo
    // and falls back to the SDK's neutral mark, so the two differ.
    expect(brandedLogo?.getAttribute('src')).not.toBe(fallbackLogo?.getAttribute('src'));
  });
...
// The model field renders a logo-bearing listbox (ModelOptionList) rather than
// a native <select>, so pick a model by clicking its option row.

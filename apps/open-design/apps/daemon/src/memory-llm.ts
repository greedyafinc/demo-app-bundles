...
async function pickProvider(projectRoot, dataDir, chatAgentId, chatProvider) {
  // Inside the UnifiedAI ecosystem every LLM call — including background
  // memory extraction — routes through the gateway with the broker token, so
  // no per-provider key is ever needed. callOpenAI's OpenAI-compatible wire
  // shape works directly against unified-api's /api/v1/chat/completions.
  if (isUnifiedConfigured()) {
    try {
      return {
        kind: 'openai',
        apiKey: await getUnifiedToken(),
        model: 'auto',
        baseUrl: `${unifiedApiBase()}/api/v1`,
        source: 'unified',
      };
    } catch (err) {
      console.warn('[memory-llm] unified token fetch failed', err?.message ?? err);
      return null;
    }
  }
  let override = null;

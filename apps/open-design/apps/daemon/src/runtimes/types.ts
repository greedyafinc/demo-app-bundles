...
export type RuntimeModelOption = {
  ...
  label: string;
  /**
   * Provider / model-author name (e.g. "Anthropic", "OpenAI"). Surfaced so the
   * web picker can render the model's brand logo. Only the platform-native
   * 'unified' agent populates it from the gateway catalog; local-CLI adapters
   * leave it undefined.
   */
  author?: string;
};

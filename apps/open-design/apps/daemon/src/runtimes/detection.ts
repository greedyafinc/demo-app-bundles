...
// The platform-native agent has no binary to probe. When the broker is
// configured we synthesize an always-available entry whose models come from
// the unified-api catalog, so the picker and chat validation are sourced from
// the gateway instead of any local CLI.
async function detectUnifiedAgent(): Promise<DetectedAgent> {
  const def = getAgentDef('unified') ?? unifiedAgentDef;
  let models: RuntimeModelOption[];
  let modelsSource: RuntimeModelSource;
  try {
    const fetched = await fetchUnifiedTextModels();
    if (fetched.length > 0) {
      models = fetched;
      modelsSource = 'live';
    } else {
...
export async function detectAgents(
  ...
  // Inside the UnifiedApp ecosystem all LLM usage goes through the gateway:
  // surface only the platform-native agent and skip probing local CLIs.
  if (isUnifiedConfigured()) {
    const unified = await detectUnifiedAgent();
    rememberLiveModels(unified.id, unified.models);
    return [unified];
  }
  const results = await Promise.all(
    AGENT_DEFS.filter((def) => def.id !== 'unified').map((def) =>
      probe(def, configuredEnvByAgent?.[def.id] ?? {}),
    ),
  );

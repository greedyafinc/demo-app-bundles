...
import { runUnifiedAgent } from './unified-agent.js';
import { isUnifiedConfigured } from './unified-auth.js';
import { getUnifiedClient } from './unified-client.js';
import { UnifiedError } from '@unifiedai/sdk/browser';
...
export async function startServer({
  ...
  // Minimal runtime shape check for the gateway's usage payload, mirroring the
  // UnifiedUsage contract's required numeric fields. Defensive only — the gateway
  // is trusted, but a schema drift or error envelope must not reach the web
  // client (which feeds it into summarizeUsage() unconditionally).
  const isValidUnifiedUsage = (u) => {
    if (!u || typeof u !== 'object') return false;
    const { plan, period, daily, credits } = u;
    const num = (v) => typeof v === 'number' && Number.isFinite(v);
    return (
      !!plan && typeof plan === 'object' && typeof plan.name === 'string' &&
      !!period && typeof period === 'object' &&
      num(period.input_tokens) && num(period.output_tokens) &&
      num(period.request_count) && num(period.cost) &&
      typeof period.resets_at === 'string' &&
      !!daily && typeof daily === 'object' &&
      num(daily.used) && num(daily.limit) && typeof daily.resets_at === 'string' &&
      !!credits && typeof credits === 'object' && num(credits.balance)
    );
  };

  // Proxy the signed-in user's account usage from the UnifiedAI gateway. Only
  // available inside the UnifiedApp desktop host (the daemon authenticates with
  // its broker token; the browser never holds a credential). Returns 404 when
  // the broker isn't configured so the web client simply hides its usage chip.
  // Response shape: { usage: UnifiedUsage } — see @open-design/contracts.
  app.get('/api/unified/usage', async (_req, res) => {
    if (!isUnifiedConfigured()) {
      return res.status(404).json({ error: 'unified_not_configured' });
    }
    try {
      // The SDK auto-retries a 401 with a force-minted broker token before
      // throwing, and parses the body into the typed UsageResponse for us.
      const usage = await getUnifiedClient().usage.get();
      // Validate the upstream shape before relaying. The web client casts the
      // body to UnifiedUsage and feeds it straight into the SDK's
      // summarizeUsage(), which destructures plan/period/daily/credits — a
      // malformed gateway response would otherwise crash the chip at render.
      // Reject as a bad gateway so the client treats it as a transient error.
      if (!isValidUnifiedUsage(usage)) {
        return res.status(502).json({ error: 'unified_usage_malformed' });
      }
      return res.json({ usage });
    } catch (err) {
      // Surface auth failures as 401 (the web treats them as "signed out");
      // everything else collapses to a 502 bad-gateway.
      const status = err instanceof UnifiedError && err.status === 401 ? 401 : 502;
      return res.status(status).json({ error: `unified_usage_failed: ${String(err)}`.trim() });
    }
  });
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // ── UnifiedAI in-daemon agent ──────────────────────────────────────────
    // The platform-native agent has no CLI to spawn: the daemon runs the
    // agent loop here against unified-api (broker-authed), reusing this run's
    // SSE channel and lifecycle. Everything below this block (MCP config
    // writes, binary resolution, spawn, stream parsing) is CLI-only and is
    // skipped for this path.
    if (def.streamFormat === 'unified-http') {
      const send = (event: string, data: unknown) => design.runs.emit(run, event, data);
      const unifiedAbort = new AbortController();
      // design.runs.cancel() prefers acpSession.abort() over killing a child
      // process; expose our HTTP abort the same way so /api/runs/:id/cancel
      // tears down the in-flight gateway request.
      run.acpSession = { abort: () => unifiedAbort.abort() };
      send('start', {
        runId,
        agentId: def.id,
        bin: def.bin,
      ...
      try {
        const result = await runUnifiedAgent({
          ...
          cwd,
          signal: unifiedAbort.signal,
          emit: (ev) => send('agent', ev),
        });
        if (run.cancelRequested || result.canceled) {
          return design.runs.finish(run, 'canceled', null, 'SIGTERM');
        }
        if (!result.ok || !result.producedOutput) {
          send(
            ...
            createSseErrorPayload(
              ...
              result.error ||
                'The model returned an empty response. Check your UnifiedAI plan limits and try again.',
              { retryable: true },
            ),
          );

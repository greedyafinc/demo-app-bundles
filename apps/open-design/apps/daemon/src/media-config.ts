...
export async function resolveProviderConfig(projectRoot: string, providerId: string): Promise<ProviderEntry> {
  // Inside the UnifiedAI ecosystem the OpenAI media surface (image generation
  // + text-to-speech) routes through the gateway with the broker token, so
  // the user never supplies an OpenAI key. unified-api is OpenAI-compatible
  // at /api/v1/images/generations and /api/v1/audio/speech — exactly what the
  // `openai` renderers post to ({baseUrl}/images/generations, etc.).
  if (providerId === 'openai' && isUnifiedConfigured()) {
    try {
      return { apiKey: await getUnifiedToken(), baseUrl: `${unifiedApiBase()}/api/v1` };
    } catch {
      // Fall through to normal resolution if the broker is unreachable.
    }
  }

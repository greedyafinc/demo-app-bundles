...
export async function finalizeDesignPackage(
  ...
  // Inside the UnifiedAI ecosystem, the DESIGN.md synthesis call routes
  // through the gateway with the broker token regardless of any BYOK creds
  // the request carried. unified-api is OpenAI-compatible at
  // /api/v1/chat/completions.
  let unifiedToken: string | null = null;
  if (isUnifiedConfigured()) {
    try {
      unifiedToken = await getUnifiedToken();
    } catch {
      unifiedToken = null;
    }
  }
  const protocol = unifiedToken ? 'openai' : options.protocol ?? 'anthropic';
  const baseUrl = unifiedToken
    ? `${unifiedApiBase()}/api/v1`
    : options.baseUrl ?? defaultBaseUrlForFinalizeProtocol(protocol);
  const apiKey = unifiedToken ?? options.apiKey;
  const model = unifiedToken ? options.model || 'auto' : options.model;

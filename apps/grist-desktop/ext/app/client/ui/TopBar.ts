// UnifiedAI override of core's app/client/ui/TopBar.ts (selected via
// ext/buildtools/webpack.config.js alias). Verbatim copy of grist-core v0.3.12's
// TopBar plus one addition: buildUnifiedAIMenu() in the document top bar, before
// the account widget. Keep in sync with upstream on version bumps.
...
import { buildUnifiedAIMenu } from "app/client/ui/UnifiedAIMenu";
...
export function createTopBarDoc(owner: MultiHolder, appModel: AppModel, pageModel: DocPageModel) {
  ...
  return [
    ...
    // UnifiedAI: AI Assistant model picker + sign-in/out (state-based).
    buildUnifiedAIMenu(owner),

...
import { setupUnifiedAI } from "app/electron/unified";
...
async function main() {
  ...
  if (exitCode) {
    ...
    try {
      setupLogging();
      // Route Grist's AI Assistant through the UnifiedAI gateway (sets the
      // ASSISTANT_* env vars) before the grist-core server boots.

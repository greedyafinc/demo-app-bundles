...
// UnifiedAI account menu actions. The unified module is the compiled TS sibling
// (app/electron/unified); require it lazily so menu construction never fails if
// the integration is absent.
async function unifiedSignIn() {
  try {
    await require('./unified').signInUnified();
    electron.dialog.showMessageBox({
      type: 'info',
      message: 'Signed in to UnifiedAI.',
      detail: "Grist's AI Assistant now runs on your Unified subscription.",
    });
  } catch (e) {
    electron.dialog.showErrorBox('UnifiedAI sign-in failed', String((e && e.message) || e));
  }
}

async function unifiedSignOut() {
  try {
    await require('./unified').signOutUnified();
    electron.dialog.showMessageBox({ type: 'info', message: 'Signed out of UnifiedAI.' });
  } catch (e) {
    electron.dialog.showErrorBox('UnifiedAI sign-out failed', String((e && e.message) || e));
  }
}
    ...
    menuTemplate.push({
      ...
      submenu: (
        ...
        .concat({
          ...
          { type: 'separator' },
          { label: 'Sign in to UnifiedAI', click: () => unifiedSignIn() },
          { label: 'Sign out of UnifiedAI', click: () => unifiedSignOut() },
        ])
      )
    });

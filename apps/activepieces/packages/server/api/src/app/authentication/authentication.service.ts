...
export const authenticationService = (log: FastifyBaseLogger) => ({
    ...
    async signInWithPassword(params: SignInWithPasswordParams): Promise<AuthenticationResponse> {
        ...
        let platformId = isNil(params.predefinedPlatformId) ? await getPreferredPlatformId(identity.id, log) : params.predefinedPlatformId

        // UnifiedApp bundle local-autologin: a Community sign-in normally returns an
        // ONBOARDING token (getPreferredPlatformId is null for CE), forcing the user
        // through the /create-platform onboarding screen. For the bundled single-user
        // appliance we resolve the identity's platform — creating it on first run — so
        // sign-in returns a full session token and the app opens straight to the
        // dashboard. Gated on AP_LOCAL_AUTOLOGIN so stock behaviour is unchanged.
        if (isNil(platformId) && process.env.AP_LOCAL_AUTOLOGIN === 'true') {
            const platforms = await platformService(log).listPlatformsForIdentityWithAtleastProject({ identityId: identity.id })
            if (platforms.length > 0) {
                platformId = platforms[0].id
            }
            else {
                return platformService(log).createPlatformWithProject({ identityId: identity.id, name: 'Local', invalidatePreviousTokens: false })
            }
        }

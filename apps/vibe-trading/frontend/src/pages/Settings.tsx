...
import { Database, KeyRound, Loader2, RotateCcw, Save, Server, ShieldCheck, SlidersHorizontal, Sparkles } from "lucide-react";
...
export function Settings() {
  ...
  // When launched inside the UnifiedApp desktop, the LLM credential is supplied
  // automatically by the broker — hide the provider/key controls and the
  // remote-deploy server-key card, mirroring OpenDesign's zero-config flow.
  const managed = settings.managed ?? false;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      {!managed && localApiAccessSection}

      <div className="space-y-2">
        <h2 className="text-lg font-semibold tracking-tight">{"LLM Settings"}</h2>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {managed
            ? "Your Unified subscription powers the agent automatically. The settings below are optional fine-tuning."
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            {managed ? <ShieldCheck className="h-4 w-4 text-primary" /> : <Server className="h-4 w-4 text-primary" />}
            <h2 className="text-base font-semibold">{managed ? "Model" : "Connection"}</h2>
          </div>
            ...
            <div className="grid gap-4">
              <div className="rounded-md border border-primary/30 bg-primary/5 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-foreground">{settings.managed_label || "UnifiedAI"}</span>
                </div>
                <p className="mt-1.5 text-sm text-muted-foreground">
                  {"Authenticated automatically through your Unified subscription — no API key to manage. The gateway auto-routes every request to an appropriate model."}
                </p>
              </div>
              <div className="grid gap-2 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-muted-foreground">{"Provider"}</span>
                  <span className="font-medium text-foreground">{settings.managed_label || form.provider}</span>
                </div>
                <div className="flex items-center justify-between gap-3">
                  <span className="text-muted-foreground">{"Model"}</span>
                  <span className="font-mono text-foreground">{form.model_name === "auto" ? "auto · gateway-routed" : form.model_name}</span>
                </div>
              </div>
            </div>
          ) : (

...
def _build_env_search() -> tuple[list[Path], tuple[str, ...]]:
    """Build the index-aligned (.env candidates, redacted labels).

    Base order: ~/.vibe-trading/.env → agent/.env → $CWD/.env. When the desktop
    host hands us a writable data dir (``VIBE_TRADING_DATA_DIR``, set by
    launch.py from ``OD_DATA_DIR``), the in-app Settings UI persists the project
    ``.env`` there — the install tree is read-only — so that slot must be
    searched FIRST. In a plain checkout the var is unset and the historical
    three-slot order is preserved exactly. The lists stay index-aligned so
    _redact_env_source keeps mapping every candidate to a leak-free label.
    """
    candidates = [
    ...
    # CWE-209: labels never contain the absolute path (OS username / home / CWD).
    labels = ["~/.vibe-trading/.env", "<AGENT_DIR>/.env", "<CWD>/.env"]
    data_dir = os.environ.get("VIBE_TRADING_DATA_DIR", "").strip()
    if data_dir:
        data_env = Path(data_dir).expanduser() / ".env"
        if data_env not in candidates:
            candidates.insert(0, data_env)
            labels.insert(0, "<DATA_DIR>/.env")
    return candidates, tuple(labels)


# Index-aligned. CWE-209: never log the absolute .env path. The label names
# which slot won - the entire P08 R1 signal - using leak-free constants only.
_ENV_CANDIDATES, _ENV_LABELS = _build_env_search()
...
def _sync_provider_env() -> None:
    ...
    _PROVIDER_MAP: dict[str, tuple[str | None, str]] = {
        "unifiedai":  ("UNIFIED_API_KEY",    "UNIFIED_API_BASE_URL"),

...
def swarm_runs_root() -> Path:
    ...
    When packaged as a UnifiedApp bundle the install tree is read-only and the
    host supplies a writable dir via ``VIBE_TRADING_DATA_DIR`` (mirrored from
    ``OD_DATA_DIR`` by launch.py); honor it here so the store and the run-dir
    allow-list both relocate together. Falls back to ``<agent_root>/.swarm/runs``.
    """
    import os

    override = os.environ.get("VIBE_TRADING_DATA_DIR", "").strip()
    base = Path(override).expanduser().resolve() if override else Path(__file__).resolve().parents[2]

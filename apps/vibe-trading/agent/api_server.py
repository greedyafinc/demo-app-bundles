...
def _writable_data_root() -> Path:
    """Root for run/session/upload data that the server writes at runtime.

    A plain source checkout (or Docker run) writes alongside the code under
    ``agent/``. When packaged as a UnifiedApp marketplace bundle the install
    tree is read-only and the desktop host hands us a writable directory via
    ``OD_DATA_DIR`` — ``launch.py`` mirrors that into ``VIBE_TRADING_DATA_DIR``
    so this stays a single, generic knob. Falls back to ``AGENT_DIR`` when
    unset, preserving the historical layout.
    """
    override = os.environ.get("VIBE_TRADING_DATA_DIR", "").strip()
    base = Path(override).expanduser().resolve() if override else AGENT_DIR
...
# Settings (.env) persist to the writable data root so in-app provider/data-source
# changes work even when the install tree is read-only. In a plain checkout
# _DATA_ROOT == AGENT_DIR, so this stays the historical agent/.env.
...
class LLMSettingsResponse(BaseModel):
    ...
    # True when launched by the UnifiedApp desktop host: the LLM credential is
    # supplied automatically by the broker and the settings UI hides the
    # provider/key controls (OpenDesign-style zero-config). managed_label is the
    # human-facing provider name to show in that read-only state.
    managed: bool = False
...
_FRONTEND_DIST = Path(
    os.environ.get("VIBE_TRADING_FRONTEND_DIST")
...
# Env vars the agent actually resolves at call time (build_llm/_sync_provider_env).
# The settings display must mirror these from the live process env, not only the
# saved .env — see _effective_settings_values.
_RUNTIME_OVERRIDE_KEYS: set[str] = {
    ...
    "VIBE_TRADING_SSE_TIMEOUT", "OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENAI_API_BASE",
}
for _provider in LLM_PROVIDERS:
    if _provider.api_key_env:
        _RUNTIME_OVERRIDE_KEYS.add(_provider.api_key_env)
    _RUNTIME_OVERRIDE_KEYS.add(_provider.base_url_env)


def _is_managed() -> bool:
    """True when launched by the UnifiedApp desktop host (managed LLM mode).

    launch.py sets ``VIBE_TRADING_MANAGED=1`` and wires the broker-minted Unified
    token into the process env. In that mode the provider/credentials are managed
    automatically, so the settings UI hides the provider/key controls (parity with
    OpenDesign's zero-config experience).
    """
    return os.environ.get("VIBE_TRADING_MANAGED", "").strip().lower() in {"1", "true", "yes"}


def _effective_settings_values() -> Dict[str, str]:
    """Settings as the agent will actually resolve them.

    build_llm()/_sync_provider_env() load .env with ``override=False``, so the
    live process environment wins over the saved file. The settings display must
    mirror that precedence — otherwise a bundle (where launch.py injects the
    Unified provider into the env, not the file) would show a stale
    ``openai / not configured`` instead of the active managed provider.

    Scoped to managed (desktop-hosted) mode: only there does the process env
    carry the authoritative broker-injected credential. In a plain checkout the
    saved file stays the single source of truth, which also keeps settings reads
    hermetic for tests.
    """
...
def _build_llm_settings_response(values: Optional[Dict[str, str]] = None) -> LLMSettingsResponse:
    ...
    Defaults to the *effective* settings (live process env wins over the saved
    .env, mirroring build_llm) so a managed bundle reflects the active Unified
    provider rather than the stale on-disk default.
    """
    env_values = values if values is not None else _effective_settings_values()
    ...
    return LLMSettingsResponse(
        ...
        managed=_is_managed(),
        managed_label=provider.label if _is_managed() else None,
    )
...
async def update_llm_settings(payload: UpdateLLMSettingsRequest):
    ...
    if _is_managed():
        # The provider + credentials are managed by the UnifiedApp host broker.
        # Pin provider/model/base to the live broker-injected runtime values and
        # never persist or clear the injected token — a stray write (or a blank
        # "clear") would otherwise wipe OPENAI_API_KEY/UNIFIED_API_KEY. Only the
        # generation params below stay user-tunable. (The UI already hides these
        # controls; this is the server-side backstop.)
        managed_provider = LLM_PROVIDER_BY_NAME.get(
            (os.environ.get("LANGCHAIN_PROVIDER") or payload.provider).strip().lower()
        )
        if managed_provider is not None:
            payload.provider = managed_provider.name
            payload.base_url = os.environ.get(managed_provider.base_url_env) or payload.base_url
        payload.model_name = os.environ.get("LANGCHAIN_MODEL_NAME") or payload.model_name
    ...
    # Reflect the effective runtime state (process env wins over the file, e.g. a
    # managed broker token that is never written to .env), consistent with GET.
    return _build_llm_settings_response()
...
def _get_swarm_runtime():
    ...
    swarm_dir = _DATA_ROOT / ".swarm" / "runs"

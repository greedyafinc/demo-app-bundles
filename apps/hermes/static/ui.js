...
// ── uni-sdk model picker: Unified catalog + brand logos ──────────────────────
// unified-api is the sole provider when bundled by UnifiedApp, so the picker is a flat
// model list (no provider grouping/selection) sourced from uni-sdk, each model rendered
// with its brand logo from uni-sdk's getModelLogo (offline data-URIs).
function _unifiedTheme(){ return document.documentElement.classList.contains('dark')?'dark':'light'; }
function _modelLogoSrc(modelId){
  try{
    const sdk=window.UnifiedSDK; if(!sdk) return '';
    const meta=(window._unifiedModels&&window._unifiedModels.get(String(modelId||'')))||null;
    const theme=_unifiedTheme();
    if(meta&&sdk.getModelLogo){ const s=sdk.getModelLogo(meta,theme); if(s) return s; }
    if(sdk.getProviderLogo){
      const author=(typeof _providerFromModelValue==='function'?_providerFromModelValue(modelId):'')||'';
      return sdk.getProviderLogo(author,theme)||'';
    }
  }catch(_e){}
  return '';
}
function _modelLogoHtml(modelId){
  const src=_modelLogoSrc(modelId);
  return src?`<img class="model-opt-logo" src="${esc(src)}" alt="" aria-hidden="true">`:'';
}
async function _loadUnifiedCatalog(){
  // Primary: uni-sdk models.list pointed at the same-origin /api/v1/models passthrough
  // (browser CSP only allows same-origin). Fallback: plain fetch of that passthrough.
  try{
    const sdk=window.UnifiedSDK;
    if(sdk&&sdk.UnifiedAI){
      // Trusted-token mode is required by the SDK. The desktop hands an app token via the
      // URL fragment (#token=…); fall back to a placeholder since /api/v1/models is
      // unauthenticated and the same-origin passthrough injects the real server bearer.
      let _tok='';
      try{ const m=(location.hash||'').match(/(?:^|[#&])token=([^&]+)/); if(m) _tok=decodeURIComponent(m[1]); }catch(_e){}
      const ai=new sdk.UnifiedAI({apiUrl:location.origin, token:_tok||'unifiedapp'});
      const res=await ai.models.list({include:['author']});
      const data=(res&&res.data)||[];
      if(data.length) return data.filter(m=>!m.type||m.type==='text');
    }
  }catch(_e){ /* fall through to plain fetch */ }
  try{
    const r=await fetch(new URL('api/v1/models?include=author',document.baseURI||location.href).href,{credentials:'include'});
    if(r.ok){ const jd=await r.json(); const data=(jd&&jd.data)||[]; return data.filter(m=>!m.type||m.type==='text'); }
  }catch(_e){}
  return [];
}
...
async function populateModelDropdown(opts={}){
  ...
  try{
    ...
    // unified-api is the sole provider — no per-provider "configured" badges/grouping.
    ...
    // Flat Unified catalog via uni-sdk — one synthetic group with no provider label so the
    // dropdown renders a single flat model list (no provider headings). Falls back to the
    // legacy provider groups if the catalog can't be loaded.
    const _unifiedCatalog=await _loadUnifiedCatalog();
    window._unifiedModels=_unifiedCatalog.length
      ? new Map(_unifiedCatalog.map(m=>[String(m.id),m]))
      : null;
    const groups=_unifiedCatalog.length
      ? [{provider:'',provider_id:null,models:_unifiedCatalog.map(m=>({id:m.id,label:m.name||m.id}))}]
      : ((Array.isArray(data.groups)&&data.groups.length)?data.groups:_synthGroupsFromConfigured());
    ...
    // Kick off a background live-model fetch only when we DIDN'T already load the full
    // Unified catalog via uni-sdk (legacy provider-probe fallback path).
    if(!window._unifiedModels && data.active_provider) _fetchLiveModels(data.active_provider, sel);
  }catch(e){
...
function syncModelChip(){
  ...
  // Swap the generic chip icon for the selected model's uni-sdk brand logo.
  const _logoEl=$('composerModelLogo');
  const _iconEl=chip.querySelector('.composer-model-icon');
  const _logoSrc=_modelLogoSrc(sel.value||'');
  if(_logoEl){
    if(_logoSrc){ _logoEl.src=_logoSrc; _logoEl.hidden=false; if(_iconEl) _iconEl.style.display='none'; }
    else { _logoEl.hidden=true; _logoEl.removeAttribute('src'); if(_iconEl) _iconEl.style.display=''; }
  }
...
function renderModelDropdown(){
  ...
  const _filterModels=(term)=>{
    ...
    for(const m of _modelData){
      ...
      // unified-api is the sole provider → no per-row provider chip; show the brand logo.
      row.innerHTML=`${_modelLogoHtml(m.value)}<div class="model-opt-main"><div class="model-opt-top"><span class="model-opt-name">${esc(m.name)}</span>${badgeHtml}</div><span class="model-opt-id">${esc(m.id)}</span></div>`;

...
def _handle_unified_catalog(handler, parsed) -> bool:
    """Proxy the Unified gateway's OpenAI-compatible /api/v1/models?include=author.

    The browser model picker (uni-sdk) can only fetch same-origin under the enforced CSP,
    so we forward server-side to the gateway the launcher configured (UNIFIED_API_BASE_URL +
    UNIFIED_API_KEY; falls back to OPENAI_*). /models is unauthenticated upstream, but we
    pass the bearer when present. Returns the OpenAI list shape {object:'list', data:[...]}.
    """
    import os as _os
    ...
    base = (_os.environ.get("UNIFIED_API_BASE_URL") or _os.environ.get("OPENAI_BASE_URL") or "").rstrip("/")
    if not base:
        return j(handler, {"object": "list", "data": [], "error": "gateway_not_configured"})
    include = _pq(parsed.query or "").get("include", [""])[0].strip()
    url = f"{base}/models" + (f"?include={include}" if include in ("author",) else "")
    req = _ur.Request(url, headers={"accept": "application/json"})
    tok = _os.environ.get("UNIFIED_API_KEY") or _os.environ.get("OPENAI_API_KEY") or ""
    if tok:
    ...
    except (_ue.URLError, OSError, ValueError) as exc:
        return j(handler, {"object": "list", "data": [], "error": str(exc)}, status=502)
...
def handle_get(handler, parsed) -> bool:
    ...
    # Same-origin passthrough to the Unified gateway's OpenAI-compatible /api/v1/models so
    # the uni-sdk-powered model picker (browser; CSP connect-src 'self') can list the
    # catalog with brand logos. unified-api is the sole provider when bundled by UnifiedApp.
    if parsed.path == "/api/v1/models":
        return _handle_unified_catalog(handler, parsed)

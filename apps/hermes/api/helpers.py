...
def _security_headers(handler):
    ...
    # X-Frame-Options: DENY is the right default for a directly-/SSH-exposed WebUI, but it
    # blocks embedding inside the UnifiedApp desktop marketplace, which renders installed
    # apps in a loopback iframe (AppHost.vue). When HERMES_WEBUI_ALLOW_FRAMING is set (the
    # bundled launcher sets it; the service binds 127.0.0.1 only), omit the header so the
    # host iframe can display us. Default (unset) preserves the upstream DENY behavior.
    if os.environ.get('HERMES_WEBUI_ALLOW_FRAMING', '').strip().lower() not in ('1', 'true', 'yes'):

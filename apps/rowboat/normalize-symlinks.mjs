#!/usr/bin/env node
// Repair pnpm-deploy symlinks that escape the bundle root.
//
// `pnpm deploy --legacy` copies workspace self-links such as
//   apps/daemon/node_modules/.pnpm/node_modules/@open-design/daemon
//     -> ../../../../../../../../apps/daemon
// with a `../` count computed for the ORIGINAL (deeper) monorepo layout. In the
// shallower staged bundle those targets climb above the bundle root, so the
// UnifiedApp installer's symlink-containment guard (symlink_target_within in
// installer.rs) rejects the *entire* archive and the install fails with

...
import { setupUnifiedAI } from './unified';
...
app
  ...
  .then(async () => {
    ...
    setupPilesFolder();
    // Route Pile's AI through the UnifiedAI gateway before the window loads, so
    // the renderer reads the pre-seeded baseUrl/key on first paint.
    await setupUnifiedAI();
    createWindow();

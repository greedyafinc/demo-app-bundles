 ...
 *
 * The SDK is imported directly from its `@unifiedai/sdk/node` exports subpath:
 * Pile's main process is bundled by webpack (target electron-main), which honors
 * the package "exports" map, so no CommonJS shim is needed (unlike grist-core's
 * classic-resolution/CJS build). The SDK's only native dependency,
 * @napi-rs/keyring, is marked external in webpack.config.base and never loaded
 * at runtime because we pass a file-backed keychain adapter below.
 *
...
// The subset of @unifiedai/sdk/node's keychain contract we depend on. Declared
// locally (rather than imported) so the type surface stays stable and the
// transpile-only build never depends on the SDK re-exporting these names.
type TokenSet = {
  accessToken: string;
  refreshToken?: string;
  expiresAt?: number;
...
const DEFAULT_CLIENT_ID = 'pile-desktop';
...
// while so a stream of AI requests can't open a stack of browser tabs. An
// already-cached session still short-circuits via isUnifiedSignedIn() before
...
/** OpenAI-compatible base URL on the gateway (chat/completions, embeddings ...). */
...
class PileUnifiedAI extends UnifiedAI {
...
let sdk: PileUnifiedAI | null = null;
...
export function getUnifiedSdk(): PileUnifiedAI {
  if (!sdk) {
    ...
    sdk = new PileUnifiedAI({
      apiUrl: unifiedApiHost(),
      appId: unifiedClientId(),
      keychain: fileKeychain,
      authorizeUrl:
        process.env.UNIFIEDAI_AUTHORIZE_URL || (local ? `${webBase}/oauth/authorize` : undefined),
...
// -- Sign-in surface (used by the proxy) -------------------------------------
 ...
 * concurrent AI requests share one sign-in attempt, and backs off after a
 * failed interactive prompt to avoid opening a stack of browser tabs. Returns
 * whether a session is available afterwards.
 */

...
class PileEmbeddings {
  ...
  async generateEmbedding(document) {
    ...
    if (isOllama) {
      ...
      // Read the configured OpenAI-compatible base URL so embeddings flow
      // through the same endpoint as chat — for the UnifiedAI build this is the
      // loopback gateway proxy (see src/main/unified). Falls back to OpenAI.
      const baseUrl =
        (await settings.get('baseUrl')) || 'https://api.openai.com/v1';
